dplyr::across(): easy summarising in R
================
...
dplyr is useful because I can use the `r group_by()` function to group a
dataframe by a specific column, then use summarise() (or mutate(), but I
will focus on the former) to iterate each of those groupings to perform
specific functions.
...
However, often times I want to calculate multiple sumary statistics
(mean, median, nth percentile, standard error, standard deviation, etc.)
...
.cols can take <tidy-select> columns, so you can give it a specific
vector of strings matching the columns you want, or its complement using
!.
...
calculations on, not necessarily summary statistics. One last thing to
mention about the across() function is that you can use purrr-style
...
formula. The contents of the grouped columns will be passed to wherever
